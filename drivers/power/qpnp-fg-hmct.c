...
static int fg_debug_mask = BIT(2) | BIT(5) | BIT(6) | BIT(7);
module_param_named(
...
static bool vbat_is_dead;
bool qpnp_vbat_is_dead(void)
{
	return vbat_is_dead;
}
EXPORT_SYMBOL(qpnp_vbat_is_dead);
...
static void update_sram_data(struct fg_chip *chip, int *resched_ms)
{
	...
	for (i = 1; i < FG_DATA_MAX; i++) {
		...
		case FG_DATA_CPRED_VOLTAGE:
			...
			if (i == FG_DATA_OCV)
				pr_info("ocv %d\n", fg_data[i].value);
			if (i == FG_DATA_VOLTAGE)
				pr_info("voltage %d\n", fg_data[i].value);
			if (i == FG_DATA_CPRED_VOLTAGE)
				pr_info("cpred_voltage %d\n", fg_data[i].value);
			break;
...
wait:
	...
	fg_data[0].value = (temp * TEMP_LSB_16B / 1000)
		- DECIKELVIN;
	if (fg_data[0].value >= MSM_ANDROID_SHDN_TEMP)
...
static int fg_set_resume_soc(struct fg_chip *chip, u8 threshold)
{
	...
	if (rc)
		...
		pr_info("setting resume-soc to 0x%02x\n", threshold);
...
static enum power_supply_property fg_power_props[] = {
	...
	POWER_SUPPLY_PROP_LOAD_PROFILE,
};
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_NOW_RAW:
		...
		break;
	case POWER_SUPPLY_PROP_LOAD_PROFILE:
...
static bool is_batt_psy_available(struct fg_chip *chip)
{
...
static int set_prop_enable_charging(struct fg_chip *chip, bool enable)
{
	...
	if (!is_batt_psy_available(chip)) {
		pr_err("batt psy not available yet!\n");
		return -EINVAL;
	}
...
static void status_change_work(struct work_struct *work)
{
	...
	/*huangyaling add 20160411, deal with reporting 100/full issue
	when term then batt missing->present, for chg term bit will not be cleared.*/
	if (chip->status == POWER_SUPPLY_STATUS_FULL &&
				  ...
				  const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_LOAD_PROFILE:
		if (val->intval) {
			pr_info("batt profile start loading\n");
			fg_batt_profile_init(chip);
		}
		break;
	default:
		return -EINVAL;
...
static int fg_batt_profile_init(struct fg_chip *chip)
{
	...
	/*huangyaling add, 2015-10-14,
	jira:E20T-1246, not use bat_id to detect profile, remove the code.
	*/
	#if 0
	wait:
	...
	if (!is_batt_psy_available(chip))
		pr_info("batt psy is not available\n");

	/* Disable charging for a FG cycle before calculating vbat_in_range */
	if (!chip->charging_disabled) {
		pr_info("charging_disabled is false\n");
		rc = set_prop_enable_charging(chip, false);
		if (rc)
			pr_err("Failed to disable charging, rc=%d\n", rc);

		if (boot_charger_status) {
			msleep(1500);
			update_sram_data(chip, &unused);
		} else
			goto reschedule;
