...
struct smbchg_chip {
	...
	int				usb_current_ma_lvl1;
	int             stepchg_soc;
	int				vfloat_mv;
	int				warm_fastchg_current_comp;
	int				warm_float_voltage_comp;
	int				cool_fastchg_current_comp;
	int				cool_float_voltage_comp;
	int				resume_delta_mv;
	int				safety_time;
	...
	int				chg_error_irq;
	u32             smb1351_paralllel_chg_en_gpio;
	u32             smb1351_paralllel_chg_en_gpio_flags;
	int             parallel_chg_en;
	...
	struct wake_lock     usbin_irq_wake_lock;

	/* TP frequency hopping */
#if (defined(CONFIG_TOUCHSCREEN_FT5X06_FH) || defined(CONFIG_TOUCHSCREEN_GT1X_FH))
...
extern bool qpnp_vbat_is_dead(void);
static int smbchg_debug_mask = BIT(1) | BIT(7);
module_param_named(
...
#if (defined(CONFIG_TOUCHSCREEN_FT5X06_FH) || defined(CONFIG_TOUCHSCREEN_GT1X_FH))
int usb_flag;
extern int ctp_work_with_ac_usb_plugin(int);
static void smb_ctp_work(struct work_struct *work)
{
	struct smbchg_chip *chip = container_of(work,
				struct smbchg_chip, ctp_work);
	ctp_work_with_ac_usb_plugin((chip->usb_present?1:0));
}
#endif
...
static int get_prop_batt_status(struct smbchg_chip *chip)
{
	...
	/*huangyaling add 20160411, deal with chg term when batt warm.
	1.still report charging status if warm float vol is lower than normal float vol.
	2.solve report 100/full issue when term then batt missing,for term bit will not cleared.*/
	if (reg & BAT_TCC_REACHED_BIT && chip->batt_present) {
		if (chip->batt_warm && chip->last_soc != 100)
...
static int get_prop_batt_capacity(struct smbchg_chip *chip)
{
	...
	rc = get_property_from_fg(chip,
		POWER_SUPPLY_PROP_LOAD_PROFILE, &profile_loaded);
	if (rc)
		pr_info("Couldn't get profile_loaded rc = %d\n", rc);
	rc = get_property_from_fg(chip, POWER_SUPPLY_PROP_CAPACITY, &capacity);
	if (rc) {
		pr_smb(PR_INFO, "Couldn't get capacity rc = %d\n", rc);
		capacity = DEFAULT_BATT_CAPACITY;
	}
	/*avoid capacity abnormal changing when usb inserted/removed. */
	if (chip->batt_present && chip->last_soc != -EINVAL
		&& profile_loaded) {
		/*when usb inserted, usbin_uv = 0*/
		usbin_uv = is_usbin_uv_high(chip);
		if (!usbin_uv
			&& (1 == (chip->last_soc - capacity))) {
			capacity = chip->last_soc;
			pr_info("prevent capacity decrease when usb inserted\n");
		} else if (usbin_uv
		&& (1 == (capacity - chip->last_soc))) {
			capacity = chip->last_soc;
			pr_info("prevent capacity increase when usb removed\n");
		}
	}
	if (chip->last_soc != capacity)
		pr_info("chip->last_soc=%d, capacity=%d\n",
			chip->last_soc, capacity);
...
#define MSM_ANDROID_SHDN_TEMP          680
/*huangyaling add, 2015-11-5,
filter wrong hot temp due to high curr consume
*/
static void batt_temp_check(struct smbchg_chip *chip,
										int *batt_temp)
{
	long interval_ot = 0;
	...
	*/
	if (*batt_temp >= MSM_ANDROID_SHDN_TEMP) {
		smbchg_stay_awake(chip, PM_TEMP_CHECK);
		get_monotonic_boottime(&ts);
		interval_ot = ts.tv_sec - last_ot_secs;
		pr_smb(PR_INFO, "batt_temp=%d, ts.tv_sec=%ld, last_ot_secs=%ld\n",
			*batt_temp, ts.tv_sec, last_ot_secs);

		if (battery_hot_count <= MSM_BAT_TEMP_SDN_COUNT)
			*batt_temp = MSM_ANDROID_SHDN_TEMP - 20;
		if (interval_ot >= 1) {
			battery_hot_count++;
		...
		battery_hot_count = 0;
		smbchg_relax(chip, PM_TEMP_CHECK);
	}

	pr_smb(PR_STATUS, "get_bat_temp: last:%d repo:%d count:%d\n",
		last_temp, *batt_temp, battery_hot_count);
	last_temp = *batt_temp;
		...
		enum battchg_enable_reason reason, bool *changed)
{
	...
	*changed = true;
	...
	pr_smb(PR_INFO, "batt charging %s, battchg_disabled = %02x\n",
		...
		enum enable_reason reason, bool *changed)
{
	...
	*changed = true;
	...
	pr_smb(PR_INFO, "usb charging %s, suspended = %02x\n",
			suspended == 0 ? "enabled"
			: "suspended", suspended);
out:
							...
							int current_ma)
{
	...
	pr_smb(PR_INFO, "USB current_ma = %d\n", current_ma);
...
out:
	pr_smb(PR_INFO, "usb current set to %d mA\n",
...
static bool smbchg_is_parallel_usb_ok(struct smbchg_chip *chip)
{
	...
	if (chip->parallel.current_max_ma == 0
		...
		pr_smb(PR_INFO, "Only been %lld since disable, skipping\n",
				ktime_to_ms(kt_since_last_disable));
		return false;
	}

	if (get_prop_charge_type(chip) != POWER_SUPPLY_CHARGE_TYPE_FAST) {
		pr_smb(PR_INFO, "Not in fast charge, skipping\n");
		return false;
	}

	if (get_prop_batt_health(chip) != POWER_SUPPLY_HEALTH_GOOD) {
		pr_smb(PR_INFO, "JEITA active, skipping\n");
		return false;
	}
	...
	if (get_usb_supply_type(type) == POWER_SUPPLY_TYPE_USB_CDP) {
		pr_smb(PR_INFO, "CDP adapter, skipping\n");
		return false;
	}

	if (get_usb_supply_type(type) == POWER_SUPPLY_TYPE_USB) {
		pr_smb(PR_INFO, "SDP adapter, skipping\n");
		return false;
	}
	...
	if (!!(reg & USBIN_SUSPEND_STS_BIT) ||
		...
		pr_smb(PR_INFO, "USB not active power source: %02x\n", reg);
		return false;
	}

	min_current_thr_ma = smbchg_get_min_parallel_current_ma(chip);
	if (min_current_thr_ma <= 0) {
		pr_smb(PR_INFO, "parallel charger unavailable for thr: %d\n",
				min_current_thr_ma);
		return false;
	}
#if 0
	if (chip->usb_tl_current_ma < min_current_thr_ma) {
		pr_smb(PR_INFO, "Weak USB chg skip enable: %d < %d\n",
							...
							int current_ma)
{
	...
	pr_smb(PR_INFO, "fastcharge current requested %d, set to %d\n",
...
static int smbchg_smb1351_parallel_chg_en(struct smbchg_chip *chip, int enable);
static void smbchg_parallel_usb_disable(struct smbchg_chip *chip)
{
	...
	if (!parallel_psy || !chip->parallel_charger_detected)
		return;
	pr_smb(PR_INFO, "disabling parallel charger\n");
	/*disable smb1351 charging by pin*/
	smbchg_smb1351_parallel_chg_en(chip, 0);
...
try_again:
	...
	pr_smb(PR_INFO, "reducing FCC of parallel charger to %d\n",
		pval.intval);
	/* Change it to uA */
	pval.intval *= 1000;
	...
	mutex_lock(&chip->parallel.lock);
	if (chip->parallel.current_max_ma == 0) {
		pr_smb(PR_INFO, "Not parallel charging, skipping\n");
		goto done;
	}
...
/*hmct, add funtion for parrallel chg en*/
static int smbchg_smb1351_parallel_chg_en(struct smbchg_chip *chip, int enable)
{
	int rc;

	if (enable == chip->parallel_chg_en) {
		pr_smb(PR_STATUS, "enable %d -> %d, skipping\n",
				chip->parallel_chg_en, enable);
		return 0;
	}
	chip->parallel_chg_en = enable;
	/*active low,enable chg:out low;disable:out high*/
	rc = gpio_direction_output(chip->smb1351_paralllel_chg_en_gpio,
			enable?0:1);
	if (rc)
		pr_err("set_out for gpio failed\n");
...
static void smbchg_parallel_usb_enable(struct smbchg_chip *chip)
{
	...
	pr_smb(PR_INFO, "Attempting to enable parallel charger\n");
	...
	if (chip->cfg_fastchg_current_ma < PARALLEL_CHG_THRESHOLD_CURRENT) {
		pr_smb(PR_INFO, "suspend parallel charger as FCC is %d\n",
	...
	if (current_limit_ma <= 0)
		goto disable_parallel;
	pr_smb(PR_INFO, "current_limit_ma %d\n", current_limit_ma);

	if (chip->parallel.initial_aicl_ma == 0) {
		if (current_limit_ma < min_current_thr_ma) {
			pr_smb(PR_INFO, "Initial AICL very low: %d < %d\n",
				current_limit_ma, min_current_thr_ma);
			goto disable_parallel;
		}
		chip->parallel.initial_aicl_ma = current_limit_ma;

		chip->hvdcp_cur_adj_soc_init = chip->previous_soc;
		pr_smb(PR_INFO, "hvdcp init soc is %d\n",
			chip->hvdcp_cur_adj_soc_init);
	}
	...
	*/
	...
	pr_smb(PR_INFO, "thm level %d, pmi8952 il %d\n",
			chip->therm_lvl_sel, chip->parallel.current_max_ma);

	/*enable smb1351 charging by pin ctrl*/
	smbchg_smb1351_parallel_chg_en(chip, 1);
	return;
...
static void smbchg_parallel_usb_en_work(struct work_struct *work)
{
	...
	if (smbchg_is_parallel_usb_ok(chip)) {
		...
		pr_smb(PR_INFO, "parallel charging unavailable\n");
		smbchg_parallel_usb_disable(chip);
	}
	mutex_unlock(&chip->parallel.lock);
}
							...
							int current_ma)
{
	...
	mutex_lock(&chip->parallel.lock);
	pr_smb(PR_INFO, "User setting FCC to %d\n", current_ma);
								...
								int lvl_sel)
{
	...
	pr_smb(PR_INFO, "set system_levle %d\n", chip->therm_lvl_sel);
	/*huangyaling add, 2015-10-16,
	description: when set, probably smb_cur + lvl_cur > dcp_max_cur,
	then usb_uv irq fired.
	*/
	if (chip->therm_lvl_sel != (chip->thermal_levels - 1) &&
		chip->parallel.current_max_ma != 0) {
		pr_smb(PR_INFO, "firstly disable smb charging.\n");
		smbchg_smb1351_parallel_chg_en(chip, 0);
		power_supply_set_current_limit(parallel_psy,
						SUSPEND_CURRENT_MA * 1000);
	}
	/*huangyaling add, 2015-10-23,
	when thermal lvl down to 0, reinitianize start soc which is
	used in smbchg_hvdcp_current_adjust_check()
	*/
	if (!chip->therm_lvl_sel && chip->enable_hvdcp_9v) {
		chip->hvdcp_cur_adj_soc_init = get_prop_batt_capacity(chip);
		pr_smb(PR_INFO, "hvdcp init soc is %d\n",
			chip->hvdcp_cur_adj_soc_init);
	}
...
static int smbchg_hvdcp_current_adjust_check(struct smbchg_chip *chip)
{
	...
	usbin_uv = prop.intval;
	pr_info("usbin vol is %d\n", usbin_uv);
	...
	if ((chip->charger_type == POWER_SUPPLY_TYPE_USB_HVDCP)
		...
		if (chip->previous_soc <= 70) {
			pr_smb(PR_INFO, "previous_soc %d soc_init %d\n",
				chip->previous_soc, chip->hvdcp_cur_adj_soc_init);
			pr_smb(PR_INFO, "chip->usb_tl_current_ma = %d  chip->usb_target_current_ma = %d\n",
					chip->usb_tl_current_ma, chip->usb_target_current_ma);
			pr_smb(PR_INFO, "chip->cfg_fastchg_current_ma = %d\n",
					 chip->cfg_fastchg_current_ma);

			if (chip->previous_soc - chip->hvdcp_cur_adj_soc_init >= chip->stepchg_soc) {
				chip->hvdcp_cur_adj_soc_init = chip->previous_soc;

				if (chip->usb_tl_current_ma != chip->usb_target_current_ma) {
					pr_smb(PR_INFO, "firstly disable smb charging.\n");
					smbchg_smb1351_parallel_chg_en(chip, 0);
					power_supply_set_current_limit(parallel_psy,
						SUSPEND_CURRENT_MA * 1000);
				...
				} else if (chip->usb_tl_current_ma != chip->usb_current_ma_lvl1) {
					pr_smb(PR_INFO, "firstly disable smb charging.\n");
					smbchg_smb1351_parallel_chg_en(chip, 0);
					power_supply_set_current_limit(parallel_psy,
						SUSPEND_CURRENT_MA * 1000);
					...
					rc = smbchg_set_thermal_limited_usb_current_max(chip,
							chip->usb_current_ma_lvl1);
					if (rc < 0)
						dev_err(chip->dev,
							"Couldn't set usb current rc = %d\n", rc);
				}
			}
		} else if (chip->usb_tl_current_ma != chip->usb_target_current_ma) {
				if (chip->parallel.current_max_ma != 0) {
					pr_smb(PR_INFO, "firstly disable smb charging.\n");
					smbchg_smb1351_parallel_chg_en(chip, 0);
					power_supply_set_current_limit(parallel_psy,
				...
				struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < chip->thermal_levels; i++) {
		len += sprintf(buf + len, "thm_lvl%d: %u\n",
			       i, chip->thermal_mitigation[i]);
	}
	...
	const char *buf, size_t count)
{
	...
	for (i = 0; i < chip->thermal_levels; i++) {
		...
		chip->thermal_mitigation[i] = value;
		pr_info("thm_lvl%d %u \n", i, value);
		if (ret <= 1)
			break;
	}
...
static DEVICE_ATTR(thermal_mitigation, 0664, thermal_mitigation_show,
	thermal_mitigation_store);
static struct attribute *thermal_mitigation_attrs[] = {
	&dev_attr_thermal_mitigation.attr,
	NULL
};
static const struct attribute_group thermal_mitigation_attr_group = {
	.attrs = thermal_mitigation_attrs,
};
...
static void smbchg_vfloat_adjust_work(struct work_struct *work)
{
	...
	pr_smb(PR_INFO, "delta_vfloat_mv = %d, samples = %d, mvbat = %d\n",
...
static void smbchg_hvdcp_det_work(struct work_struct *work)
{
	...
	pr_smb(PR_INFO, "HVDCP_STS = 0x%02x\n", reg);
	/*
	...
	if ((reg & hvdcp_sel) && is_usb_present(chip)) {
		if (chip->enable_hvdcp_9v
			...
			/*huangyaling add,
			to solve QC2.0 charger+weak usb line not charging issue
			*/
		...
		pr_smb(PR_INFO, "setting usb psy type = %d\n",
...
static void handle_usb_removal(struct smbchg_chip *chip)
{
	...
	if (chip->usb_psy) {
		...
		pr_smb(PR_INFO, "setting usb psy allow detection 0\n");
...
static void handle_usb_insertion(struct smbchg_chip *chip)
{
	...
	/*huangyaling add, 2015-10-23, use as start soc for hvcdp current adjust work
	in smbchg_hvdcp_current_adjust_check()
	*/
	if (chip->enable_hvdcp_9v) {
		chip->hvdcp_cur_adj_soc_init = get_prop_batt_capacity(chip);
		pr_smb(PR_INFO, "hvdcp init soc is %d\n",
...
static enum power_supply_property smbchg_battery_properties[] = {
	...
	POWER_SUPPLY_PROP_CHARGE_TIMER_ENABLED,
				       ...
				       const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_SAFETY_TIMER_ENABLE:
		...
		break;
	case POWER_SUPPLY_PROP_CHARGE_TIMER_ENABLED:
		rc = smbchg_safety_timer_enable(chip, val->intval);
		break;
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_FLASH_CURRENT_MAX:
		...
		break;
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX:
		val->intval = chip->cfg_fastchg_current_ma * 1000;
		break;
	case POWER_SUPPLY_PROP_SYSTEM_TEMP_LEVEL:
...
static irqreturn_t batt_warm_handler(int irq, void *_chip)
{
	...
	pr_smb(PR_INTERRUPT, "triggered: 0x%02x\n", reg);
	/*huangyaling add, 2015-10-23,
	when battery not warm, reinitialize start soc which is
	used in smbchg_hvdcp_current_adjust_check()
       */
	if (!chip->batt_warm && chip->enable_hvdcp_9v) {
		chip->hvdcp_cur_adj_soc_init = get_prop_batt_capacity(chip);
		pr_smb(PR_INFO, "hvdcp init soc is %d\n",
			chip->hvdcp_cur_adj_soc_init);
	}
	if (chip->batt_warm) {
		if (chip->warm_float_voltage_comp != -EINVAL) {
			rc = smbchg_float_voltage_comp_set(chip,
				chip->warm_float_voltage_comp);
			if (rc < 0) {
				dev_err(chip->dev, "Couldn't set float voltage comp rc = %d\n",
					rc);
			}
		...
		if (chip->warm_fastchg_current_comp != -EINVAL) {
			rc = smbchg_fastchg_current_comp_set(chip,
				chip->warm_fastchg_current_comp);
			if (rc < 0) {
				dev_err(chip->dev, "Couldn't set fastchg current comp rc = %d\n",
					rc);
			}
...
static irqreturn_t batt_cool_handler(int irq, void *_chip)
{
	...
	pr_smb(PR_INTERRUPT, "triggered: 0x%02x\n", reg);
	/*huangyaling add, 2015-10-23,
	when battery not cool, reinitialize start soc which is
	used in smbchg_hvdcp_current_adjust_check()
       */
	if (!chip->batt_cool && chip->enable_hvdcp_9v) {
		chip->hvdcp_cur_adj_soc_init = get_prop_batt_capacity(chip);
		pr_smb(PR_INFO, "hvdcp init soc is %d\n",
			chip->hvdcp_cur_adj_soc_init);
	}
	if (chip->batt_cool) {
		if (chip->cool_float_voltage_comp != -EINVAL) {
			rc = smbchg_float_voltage_comp_set(chip,
				chip->cool_float_voltage_comp);
			if (rc < 0) {
				dev_err(chip->dev, "Couldn't set float voltage comp rc = %d\n",
					rc);
			}
		...
		if (chip->cool_fastchg_current_comp != -EINVAL) {
			rc = smbchg_fastchg_current_comp_set(chip,
				chip->cool_fastchg_current_comp);
			if (rc < 0) {
				dev_err(chip->dev, "Couldn't set fastchg current comp rc = %d\n",
					rc);
			}
...
static irqreturn_t usbin_ov_handler(int irq, void *_chip)
{
	...
	pr_smb(PR_INTERRUPT,
		"chip->usb_present = %d usb_present = %d rt_sts = 0x%02x\n",
		chip->usb_present, usb_present, reg);
out:
	return IRQ_HANDLED;
}
...
static irqreturn_t usbin_uv_handler(int irq, void *_chip)
{
	...
	bool unused;
	u8 reg;
	wake_lock_timeout(&chip->usbin_irq_wake_lock, 3 * HZ);
	...
	if (!(reg & USBIN_UV_BIT) && !(reg & USBIN_SRC_DET_BIT)) {
		pr_smb(PR_INFO, "setting usb psy allow detection 1\n");
...
#if (defined(CONFIG_TOUCHSCREEN_FT5X06_FH) || defined(CONFIG_TOUCHSCREEN_GT1X_FH))
	usb_flag = usb_present;
	schedule_work(&chip->ctp_work);
#endif
	return IRQ_HANDLED;
}
...
#define SYSMIN_MASK         SMB_MASK(1, 0)
#define SYSMIN_3P15         0
#define SYSMIN_3P60         2
...
static void batt_ov_wa_check(struct smbchg_chip *chip)
{
	...
	if (reg & BAT_OV_BIT) {
		...
		msleep(200);
		pr_smb(PR_INFO, "bat_ov, restart charging.\n");
...
static int smbchg_hw_init(struct smbchg_chip *chip)
{
	...
	if (!get_prop_batt_present(chip)) {
		...
		rc = smbchg_sec_masked_write(chip,
				chip->bat_if_base + CFG_SYSMIN,
				SYSMIN_MASK, SYSMIN_3P60);
		if (rc < 0) {
			dev_err(chip->dev, "Couldn't set SYSMIN 3600 rc=%d\n",
				rc);
			return rc;
		}
	} else {
		...
		rc = smbchg_sec_masked_write(chip,
				chip->bat_if_base + CFG_SYSMIN,
				SYSMIN_MASK, SYSMIN_3P15);
		if (rc < 0) {
			dev_err(chip->dev, "Couldn't set SYSMIN 3150 rc=%d\n",
					rc);
			return rc;
		}
	}
	...
	if (chip->enable_hvdcp_9v) {
		/*PMI2.0 take effect*/
		pr_smb(PR_INFO, "******hvdcp sel 9v.\n");
		rc = smbchg_sec_masked_write(chip,
			chip->usb_chgpth_base + CHGPTH_CFG,
			HVDCP_ADAPTER_SEL_MASK,
			HVDCP_ADAPTER_SEL_9V_BIT);
		if (rc) {
...
static int smb_parse_dt(struct smbchg_chip *chip)
{
	...
	OF_PROP_READ(chip, chip->usb_current_ma_lvl1,
			"stepchg_usb_current_ma_lvl1", rc, 1);
	OF_PROP_READ(chip, chip->stepchg_soc,
			"stepchg_soc", rc, 1);
	...
	OF_PROP_READ(chip, chip->prechg_safety_time, "precharging-timeout-mins",
			rc, 1);
	OF_PROP_READ(chip, chip->warm_fastchg_current_comp, "warm-fastchg-current-comp",
			rc, 1);
	OF_PROP_READ(chip, chip->warm_float_voltage_comp, "warm-float-voltage-comp",
			rc, 1);
	OF_PROP_READ(chip, chip->cool_fastchg_current_comp, "cool-fastchg-current-comp",
			rc, 1);
	OF_PROP_READ(chip, chip->cool_float_voltage_comp, "cool-float-voltage-comp",
			rc, 1);
	if (chip->safety_time != -EINVAL &&
    ...
    /*
	hisense code,
	add for smb1351 charge pin ctrl,using pmi8952 gpio 2
	*/
	chip->smb1351_paralllel_chg_en_gpio = of_get_named_gpio_flags(node,
			"qcom,smb1351-parallel-chg-en-gpio", 0,
			&chip->smb1351_paralllel_chg_en_gpio_flags);
	if (chip->smb1351_paralllel_chg_en_gpio < 0)
		chip->smb1351_paralllel_chg_en_gpio = -EINVAL;
...
static int smbchg_request_irqs(struct smbchg_chip *chip)
{
	...
	spmi_for_each_container_dev(spmi_resource, chip->spmi) {
		...
		case SMBCHG_LITE_BAT_IF_SUBTYPE:
			...
			enable_irq_wake(chip->batt_ov_irq);
...
static int smbchg_probe(struct spmi_device *spmi)
{
...
#if (defined(CONFIG_TOUCHSCREEN_FT5X06_FH) || defined(CONFIG_TOUCHSCREEN_GT1X_FH))
	INIT_WORK(&chip->ctp_work, smb_ctp_work);
#endif
	...
	wake_lock_init(&chip->usbin_irq_wake_lock, WAKE_LOCK_SUSPEND,
		"usbin_irq_lock");
    ...
    /*
    hmct,add pmi gpio2 control smb1351 parallel charging,
    init high,disable parallel charging
    */
	if (gpio_is_valid(chip->smb1351_paralllel_chg_en_gpio)) {
		rc = gpio_request(chip->smb1351_paralllel_chg_en_gpio,
			"smb1351_parallel_charging_en");
		if (rc)
			pr_err("gpio request failed");

		rc = gpio_direction_output(
			chip->smb1351_paralllel_chg_en_gpio, 1);
		if (rc)
			pr_err("set_direction for gpio failed\n");

		pr_err("parrallel gpio request success.\n");
	}
	...
	/*
	hmct, add device wakeup source register, fix wakeup source not function
	*/
	...
	power_supply_set_present(chip->usb_psy, chip->usb_present);
	set_property_on_fg(chip, POWER_SUPPLY_PROP_LOAD_PROFILE, 1);

	dump_regs(chip);
	create_debugfs_entries(chip);

	rc = sysfs_create_group(&chip->dev->kobj, &thermal_mitigation_attr_group);
	if (rc)
		pr_err("create thermal_mitigation device attr fail\n");
...
free_regulator:
	wake_lock_destroy(&chip->usbin_irq_wake_lock);
...
{
	struct smbchg_chip *chip = dev_get_drvdata(dev);
	pr_smb(PR_PM, "wake_reasone 0x%02x\n", chip->wake_reasons);

	if (chip->wake_reasons != 0)
		return -EBUSY;
	return 0;
}
